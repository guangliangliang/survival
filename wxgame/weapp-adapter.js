...
/***/ (function (module, exports, __webpack_require__) {
  ...
  /***/ (function (module, exports) {
        ...
        function getWxWindowInfo() {
            return wx.getWindowInfo ? wx.getWindowInfo() : wx.getSystemInfoSync()
        }

        function toNumber(value, fallback) {
            const ret = Number(value)
            return Number.isFinite(ret) && ret > 0 ? ret : fallback
        }

        const wxWindowInfo = getWxWindowInfo()
        const rawScreenWidth = toNumber(wxWindowInfo.screenWidth, wxWindowInfo.windowWidth)
        const rawScreenHeight = toNumber(wxWindowInfo.screenHeight, wxWindowInfo.windowHeight)
        const devicePixelRatio = toNumber(wxWindowInfo.pixelRatio, wxWindowInfo.devicePixelRatio || 1)

        const innerWidth = exports.innerWidth = Math.max(rawScreenWidth, rawScreenHeight)
        const innerHeight = exports.innerHeight = Math.min(rawScreenWidth, rawScreenHeight)
        const safeArea = exports.safeArea = wxWindowInfo.safeArea || {
            left: 0,
            top: 0,
            right: innerWidth,
            bottom: innerHeight,
